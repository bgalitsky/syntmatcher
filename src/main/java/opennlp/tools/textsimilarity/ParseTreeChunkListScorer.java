	...
	public double getScore(ParseTreeChunk chunk)
	{
		...
		for (String l : chunk.getLemmas())
		{
			...
			if (l.equals("*"))
			{
				...
				{ // number vs number gives high score although different numbers
					score += 0.7;
				}
				else if (pos.endsWith("_high"))
				{ // if query modification adds 'high'
					score += 1.0;
				}
				else
				{
			...
			else
			{
				...
				{
					if (l.equals("get"))
					{ // 'common' verbs are not that important
						score += 0.3;
					}
					else
					{
