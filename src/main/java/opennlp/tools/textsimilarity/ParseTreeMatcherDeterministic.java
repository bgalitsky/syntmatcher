...
public class ParseTreeMatcherDeterministic
{
	...
	public List<ParseTreeChunk> generalizeTwoGroupedPhrasesDeterministic(ParseTreeChunk chunk1, ParseTreeChunk chunk2)
	{
		...
		// if at some position correspondence is inverse (one of two position decreases instead of increases)
		...
		for (List<int[]> occur : overlapsPlaus)
		{
			...
			while (k1 <= k1max && k2 <= k2max)
			{
				...
				if ((sim != null) && (lemmaMatch == null || (lemmaMatch != null && !lemmaMatch.equals("fail"))))
				{
					...
					if (lemmaMatch != null)
					{
						...
						if (k1 == occr1.get(ov1) && k2 == occr2.get(ov2))
							bReachedCommonWord = true; // now we can have different increment opera
						else
						{
							...
							// else
							// System.err.println("Next match reached '"+lemmaMatch+ "' | k1 - k2: "+k1 + " "+k2 +
							// "| occur index ov1-ov2 "+
							// ov1+" "+ov2+ "| identified positions of match: occr1.get(ov1) - occr2.get(ov1) " +
					...
					{
						commonLemmas.add("*");
					} // the same parts of speech, proceed to the next word in both expressions
					k1++;
					k2++;
				...
				else
				{
					...
					{ // if it is behind current position, synchroneously move towards right
						kk1++;
						kk2++;
						countMove++;
					}
			...
			comp < sent1.size() && comp < sent2.size(); comp++)
		{
			...
			for (ParseTreeChunk ch1 : sent1.get(comp))
			{
				...
				{ // simpler version
					...
					// System.out.println("ch1 = "+ ch1.toString()+" | ch2="+ch2.toString()
					 ...
					 * ParseTreeMatcher.prepositionalNNSTransform(ch1), ch2); if (chunkToAdd1!=null)
					 * chunkToAdd.addAll(chunkToAdd1); List<ParseTreeChunk> chunkToAdd2 =
					 * ParseTreeMatcherDeterministic.generalizeTwoGroupedPhrasesDeterministic(
					 * ParseTreeMatcher.prepositionalNNSTransform(ch2), ch1); if (chunkToAdd2!=null)
					 * chunkToAdd.addAll(chunkToAdd2);
					 */
