...
/**
 * Created by IntelliJ IDEA. User: boris Date: Feb 13, 2009 Time: 2:18:47 PM To change this template use File | Settings
 * | File Templates.
 */
public class ParseTreeMatcher
{
	...
	public ParseTreeChunk prepositionalNNSTransform(ParseTreeChunk ch)
	{
		...
		if (indexIN < 2)// preposition is a first word - should not be in a noun phrase
			return ch;
		...
		ParseTreeChunk chunk2)
	{
		...
		// Map <ParseTreeChunk, Double> scoredResults = new HashMap <ParseTreeChunk, Double> ();
		...
		for (int timesRun = 0; timesRun < timesRepetitiveRun; timesRun++)
		{
			...
			while (k1 < pos1.size() && k2 < pos2.size())
			{
				// first check if the same POS
				String sim = posManager.similarPOS(pos1.get(k1), pos2.get(k2));
				String lemmaMatch = lemmaFormManager.matchLemmas(null, chunk1.getLemmas().get(k1), chunk2.getLemmas()
					.get(k2), sim);
