...
public class StringDistanceMeasurer
{
	...
	protected List<String> filterWordArray(String[] strWords)
	{
		...
		for (String w : strWords)
		{
			...
			if (w.length() < MIN_STRING_LENGTH_FOR_WORD && !bInteger) // only non-integer short
	...
	// returns 1 for identical strings and 0 for totally different
	public double measureStringDistance(String str1, String str2)
	{
		...
		try
		{
			...
			if (str1.equals(str2)) // || str1.endsWith(str2) || str2.endsWith(str1)) bg 03-2011
				return 1.0;
			...
			for (String w : strListOverlap)
			{
				if (w.toLowerCase().equals(w)) // no special interest word
					...
					lOverlap += 2; // if capitalized, or specific word => important so double score
			}
				...
				for (String w2 : str2List)
				{
					if (w1.length() > MIN_STRING_LENGTH_FOR_DISTORTED_WORD || !w1.toLowerCase().equals(w1))
						if (w2.length() > MIN_STRING_LENGTH_FOR_DISTORTED_WORD || !w2.toLowerCase().equals(w2))
							if (LevensteinDistanceFinder.levensteinDistance(w1, w2, 1, 10, 1, 10) <= ACCEPTABLE_DEVIATION_IN_CHAR)
								countSimilar++;
				}
			}
			...
			// double ld = LevensteinDistanceFinder. levensteinDistance(str1, str2, 1, 10, 1, 10);
		...
		/*
		 * if (result>MIN_SCORE_FOR_LING) { List<List<ParseTreeChunk>> matchResult = pos.matchOrigSentencesCache(str1,
		 * str2); linguisticScore = ParseTreeChunkListScorer.getParseTreeChunkListScore(matchResult);
		 * System.out.println(matchResult);
		 * 
		 * // magic formula for 0.7 string match and 0.3 linguistic match result = result*0.7 + linguisticScore/6.0*
		 * 0.3; }
		 */
		return result;
	}
	...
	public static void main(String[] args)
	{
		...
		// String sent1 = "estoy en LA,California y no encuentro tu album en NINGUNA parte!!! " +
		...
		String sent8dd = "Pakistani defence minister banned from travel"; // to check the case of 1.0
		...
		// [0.8178702752867737, 0.21082473737065027, 0.27594593229224296, 0.7517586466500455, 0.9100766715907641]
		...
		System.out.println(matchRes);
		// [0.48044977359257246, 0.8365116420730185, 0.8365116420730185, 0.48044977359257246, 0.27594593229224296,
		// 0.6391010941257969]
