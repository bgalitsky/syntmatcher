...
public class LevensteinDistanceFinder
{
	...
	/**
	 * Computes Levenstain distance (unit distance) between two strings. Use dynamic programming algorithm to calculate
	 * matrix with distances between substrings. Time complexity - O(length1 * length2), memory - O(length1 + length2)
	 * 
	...
	public static double distanceBetweenStringArraysAsSpaceSepar(String line1, String line2)
	{
		...
		for (int i = 0; i < strings1.length; i++)
		{
			for (int j = 0; j < strings2.length; j++)
			{
				...
				if (levensteinDistance(strings1[i], strings2[j], 1, 1, 1, 1) / (strings1.length + strings2.length) < 0.2)
				{
					intersectNum++;
					selected2[j] = true;
					selected1[i] = true;
