...
public class Utils
{
	...
	static
	{
		characterMappings.add(new String[] { "[ÃƒÂ ÃƒÂ¡ÃƒÂ¢ÃƒÂ£ÃƒÂ¤ÃƒÂ¥Ã¯Â¿Â½?Ã„Æ’Ã„â€¦Ã�Â°]", " " }); // was a
		characterMappings.add(new String[] { "[Ãƒâ‚¬Ã¯Â¿Â½?Ãƒâ€šÃƒÆ’Ãƒâ€žÃƒâ€¦Ã„â‚¬Ã„â€šÃ„â€žÃ¯Â¿Â½?]", "A" });
		characterMappings.add(new String[] { "[ÃƒÂ§Ã„â€¡Ã„â€°Ã„â€¹Ã¯Â¿Â½?]", "c" });
		characterMappings.add(new String[] { "[Ãƒâ€¡Ã„â€ Ã„Ë†Ã„Å Ã„Å’]", "C" });
		characterMappings.add(new String[] { "[Ã¯Â¿Â½?Ã„â€˜]", "d" });
		characterMappings.add(new String[] { "[Ã¯Â¿Â½?Ã„Å½Ã¯Â¿Â½?]", "D" });
		characterMappings.add(new String[] { "[ÃƒÂ¨ÃƒÂ©ÃƒÂªÃƒÂ«ÃƒÂ¦Ã„â„¢Ã„â€œÃ„â€¢Ã„â€”Ã„â„¢Ã„â€º]", " " }); // was e
		characterMappings.add(new String[] { "[ÃƒË†Ãƒâ€°ÃƒÅ Ãƒâ€¹Ãƒâ€ Ã„â€™Ã„â€�Ã„â€“Ã„ËœÃ„Å¡]", "'" }); // was E
		characterMappings.add(new String[] { "[Ã¯Â¿Â½?Ã„Å¸Ã„Â¡Ã„Â£]", "g" });
		characterMappings.add(new String[] { "[Ã„Å“Ã„Å¾Ã„Â Ã„Â¢Ã†â€œ]", "G" });
		characterMappings.add(new String[] { "[Ã„Â¥Ã„Â§]", "h" });
		characterMappings.add(new String[] { "[Ã„Â¤Ã„Â¦]", "H" });
		characterMappings.add(new String[] { "[ÃƒÂ¬ÃƒÂ­ÃƒÂ®ÃƒÂ¯Ã„Â©Ã„Â«Ã„Â­Ã„Â®Ã„Â¯Ã„Â±Ã„Â³Ã„Âµ]", "i" });
		characterMappings.add(new String[] { "[ÃƒÅ’Ã¯Â¿Â½?ÃƒÅ½Ã¯Â¿Â½?Ã„Â¨Ã„ÂªÃ„Â¬Ã„Â°Ã„Â²Ã„Â´Ã„Âµ]", "I" });
		characterMappings.add(new String[] { "[Ã„Â·Ã„Â¸]", "k" });
		characterMappings.add(new String[] { "[Ã„Â¶]", "K" });
		characterMappings.add(new String[] { "[ÃƒÂ¸Ã…â€˜ÃƒÂ°ÃƒÂ²ÃƒÂ³ÃƒÂ´ÃƒÂµÃƒÂ¶Ã¯Â¿Â½?Ã¯Â¿Â½?Ã…â€˜Ã…â€œÃ†Â¡]", "o" });
		characterMappings.add(new String[] { "[Ãƒâ€™Ãƒâ€œÃƒâ€�Ãƒâ€¢Ãƒâ€“ÃƒËœÃ…Å’Ã…Å½Ã¯Â¿Â½?Ã…â€™Ã†Â ]", "O" });
		characterMappings.add(new String[] { "[ÃƒÂ±Ã…â€žÃ…â€ Ã…Ë†Ã…â€°Ã…â€¹]", "n" });
		characterMappings.add(new String[] { "[Ãƒâ€˜Ã…Æ’Ã…â€¦Ã…â€¡Ã…Å Ã…â€¹]", "N" });
		characterMappings.add(new String[] { "[Ã„ÂºÃ„Â¼Ã„Â¾Ã…â‚¬Ã…â€š]", "l" });
		characterMappings.add(new String[] { "[Ã„Â¹Ã„Â»Ã„Â½Ã„Â¿Ã¯Â¿Â½?]", "L" });
		characterMappings.add(new String[] { "[ÃƒÂ¹ÃƒÂºÃƒÂ»ÃƒÂ¼Ã…Â©Ã…Â«Ã…Â­Ã…Â¯Ã…Â±Ã…Â³Ã†Â°]", "u" });
		characterMappings.add(new String[] { "[Ãƒâ„¢ÃƒÅ¡Ãƒâ€ºÃƒÅ“Ã…Â¨Ã…ÂªÃ…Â¬Ã…Â®Ã…Â°Ã…Â²Ã†Â¯]", "U" });
		characterMappings.add(new String[] { "[ÃƒÂ½ÃƒÂ¿Ã…Â·]", "y" });
		characterMappings.add(new String[] { "[Ã¯Â¿Â½?Ã…Â¶Ã…Â¸]", "Y" });
		characterMappings.add(new String[] { "[Ã…â€¢Ã…â€”Ã…â„¢]", "r" });
		characterMappings.add(new String[] { "[Ã…â€�Ã…â€“Ã…Ëœ]", "R" });
		characterMappings.add(new String[] { "[Ã…Â¡Ã…â€ºÃ¯Â¿Â½?Ã…Å¸Ã…Â¡Ã…Â¿]", "s" });
		characterMappings.add(new String[] { "[Ã…Â Ã…Å¡Ã…Å“Ã…Å¾Ã…Â Ã…Â¿]", "S" });
		characterMappings.add(new String[] { "ÃƒÅ¸", "ss" });
		characterMappings.add(new String[] { "ÃƒÅ¾", "th" });
		characterMappings.add(new String[] { "ÃƒÂ¾", "Th" });
		characterMappings.add(new String[] { "[Ã…Â£Ã…Â¥Ã…Â§]", "t" });
		characterMappings.add(new String[] { "[Ã…Â¢Ã…Â¤Ã…Â¦]", "T" });
		characterMappings.add(new String[] { "[Ã…Âµ]", "w" });
		characterMappings.add(new String[] { "[Ã…Â´]", "W" });
		characterMappings.add(new String[] { "[Ã…Â¾Ã…ÂºÃ…Â¼Ã…Â¾Ã†Â¶]", "z" });
		characterMappings.add(new String[] { "[Ã…Â½Ã…Â½Ã…Â¹Ã…Â»Ã…Â½Ã†Âµ]", "Z" });
	...
	public static String stripNonAsciiChars(String s)
	{
		...
		return b.toString().trim().replaceAll("\\s+", " "); // replace any multiple spaces with a single space
	}
	...
	{
		Pattern p = java.util.regex.Pattern.compile("\\<NOSCRIPT.*?</NOSCRIPT>", Pattern.DOTALL
	...
	public static String stripHTMLMultiLine(String text, HashSet<String> allowedHtmlTags, String escGtCh, String escLtCh)
	{
	...
	public static String truncateOnSpace(String text, Integer length)
	{
		...
		else
		{
			...
			{
				if (b.length() >= length && Character.isWhitespace(text.charAt(i)))
				{ // iterate until we hit whitespace
					b.append("...");
					break;
				}
	...
	static public void main(String[] args)
	{
		...
		System.out.println(isLatinWord("ãƒ‡ãƒ¼ãƒ“ãƒƒãƒ‰ãƒ»ã"));
		System.out.println(isLatinWord("Ã© Ã±Ã§Ã¸Ã¥Ã³"));
		System.out.println(isLatinWord("Ã¹Ã¬Ã®Ã¤ Ã Ã¸Ã¶Ã©"));
		System.out.println(isLatinWord("é™³æ¸¯ç”Ÿ, é™ˆæ¸¯ç”Ÿ"));
		...
		System.out.println(convertToASCII(" .             Ron Carter is among the most original, prolific. "));
		// TODO deal with www.wmot.org/program-guide/program-listings/28th_annual_playboy_jazz_festiva_2006.htm
		System.out
