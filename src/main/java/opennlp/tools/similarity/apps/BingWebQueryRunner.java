...
{
	private static final Logger LOG = LoggerFactory.getLogger(BingWebQueryRunner.class);

	private String constructBingWebUrl(String query, String domainWeb, String lang, int numbOfHits) throws Exception
	{
		String codedQuery = URLEncoder.encode(query, "UTF-8");

		String yahooRequest = "http://api.search.live.net/json.aspx?Appid=" + BingQueryRunner.APP_ID + "&query="
			+ codedQuery // +
							// "&sources=web"+
			+ "&Sources=Web"
