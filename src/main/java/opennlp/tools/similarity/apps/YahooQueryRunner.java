...
/**
 * This class is the superclass of all classes which are using yahoo-websearch API with JSON.
 * 
 */
public class YahooQueryRunner
{
	protected static final String APP_ID = "lyzaGJDV34EsJbugCymf7_oosEfMtbSwUBOhDQ8abqgy_Sl2roPjyg72T5.k1NIoyQ--";

	private static final Logger LOG = LoggerFactory.getLogger(YahooQueryRunner.class);

	/**
	 * To run a query on Yahoo, one needs the
	 * 
	 * @param query it can be a row text with some pre-processing
	 * @param domainWeb some sub-domain if necessary (default "")
	 * @param lang language settings
	 * @param numbOfHits
	 * @return
	 * @throws Exception
	 */
	protected String constructBossUrl(String query, String domainWeb, String lang, int numbOfHits) throws Exception
	{
		String _lang = "en";

		String codedQuery = URLEncoder.encode(query, "UTF-8");
		String yahooRequest = "http://boss.yahooapis.com/ysearch/web" + "/v1/" + codedQuery + "?appid=" + APP_ID
			+ "&count=" + numbOfHits + "&format=json&sites=" + domainWeb + "&lang=" + _lang;
		return yahooRequest;
	}
	 ...
	 * @param query
	 * @param domainWeb
	 * @param numbOfHits For more details http://developer.yahoo.com/search/image/V1/imageSearch.html
	 * @return
	 * @throws Exception
	 */
	protected String constructBossImageSearchUrl(String query, String domainWeb, int numbOfHits) throws Exception
	{
		String codedQuery = URLEncoder.encode(query, "UTF-8");
		String yahooRequest = "http://boss.yahooapis.com/ysearch/images/v1/" + codedQuery + "?appid=" + APP_ID
			+ "&count=" + numbOfHits + "&format=json&sites=" + domainWeb;
		return yahooRequest;
	}
	...
	public ArrayList<String> searchImage(String query, String domainWeb, int numbOfHits) throws Exception
	{
	...
	protected void printResponseAttributes(JSONObject responseObject) throws Exception
	{
		...
		System.out.println("responseCode = " + responseCode + ", totalHits = " + totalHits + ", deepHits = " + deepHits
			+ ", startIndex = " + startIndex + ", pageSize = " + pageSize);
		System.out.println("nextPageUrl = " + nextPageUrl);
	}

	protected void printSingleSearchResult(JSONObject singleResult) throws Exception
	{
